...
导入本项目的其他文件import "package:${your_project}/${filepath}";
...
1. 静态字段和静态方法会先初始化。它们只初始化一次,在类被加载时完成初始化。静态字段和静态方法的初始化顺序跟它们在代码中出现的顺序一致。
2. 初始化列表会在构造函数体执行前执行。初始化列表中变量的初始化顺序跟它们在代码中出现的顺序一致。
3. 构造函数体会在初始化列表执行后开始执行。
4. 普通字段会在构造函数体执行完后初始化。它们的初始化顺序跟字段在类中定义的顺序一致。
5. 普通方法会在对象实例化时绑定到该实例,但是只有在第一次被调用时才会真正执行。
...
class _QuizState extends State<Quiz> {
    Widget? activeScreen = StartScreen(switchScreen);			//由于初始化顺序，字段初始化时，方法还未绑定，因此会报错。这是一种动态行为，本质上函数作为一等公民而存在。但是在Java中可以避免这种情况，因为Java的函数式接口本质上还是对象之间的赋值，这在编译期对象类型就已经确定，相对应的函数接口也确定下来，这是一种静态行为。但是Dart与Java的多态机制还是一样的，都是动态绑定的。
    void switchScreen() {
        setState(() {
