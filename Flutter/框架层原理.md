...
- **「Renderer Widget」** —— 渲染类 Widget，是最核心的`Widget`类型，会直接参与后面的「Layout」、「Paint」流程。无论是「Component Widget」还是「Proxy Widget」最终都会映射到「Renderer Widget」上，否则将无法被绘制到屏幕上。
...
在 `Widget`基类中有 3 个重要的方法 (属性)：

- **Key key** —— 在同一父节点下，用作兄弟节点间的唯一标识，
- **Element createElement()** —— 每个`Widget`都有一个与之对应的`Element`，由该方法负责创建
- **static bool canUpdate(Widget oldWidget, Widget newWidget)** ：是否复用之前创建的Element
...
- 通过`setState`方法之所以能更新 UI，是在其内部调用`_element.markNeedsBuild()`实现的
...
void _updateParentData(ParentDataWidget<RenderObjectWidget> parentData) {
  parentData.applyParentData(renderObject);			//ParentDataWidget需要重写applyParentData方法，以便设置对应RenderObject.parentData。
}
~~~
...
- *RenderObject createRenderObject(BuildContext context)* —— 核心方法，创建 Render Widget 对应的 Render Object。该方法在对应的 Element 被挂载到树上时调用。
- *RenderObjectElement createElement()* ——「RenderObject Widget」对应的 Element 为`RenderObjectElement`
...
对于需要更新的 element，首先会调用在`setState`方法中调用`Element.markNeedsBuild`方法
...
然后在`markNeedsBuild`方法中调用`BuildOwner.scheduleBuildFor`方法：
...
void buildScope(Element context, [ VoidCallback callback ]) {
  ...
  try {
    ...
    _dirtyElements.sort(Element._sort);				//对「dirty elements」按在「Element Tree」上的深度排序 
    int dirtyCount = _dirtyElements.length;
    int index = 0;
    while (index < dirtyCount) {
      _dirtyElements[index].rebuild();				//对_dirtyElements中的元素依次调用rebuild
      index += 1;
    }
  } finally {
    ...
    _dirtyElements.clear();							//清理_dirtyElements 
  }
}
~~~
...
- RenderObjectElement
  ...
  void update(covariant RenderObjectWidget newWidget) {
    super.update(newWidget);
    //调用了widget.updateRenderObject来更新「Render Object」
