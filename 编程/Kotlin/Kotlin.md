...
异步编程有很多模型，包括：线程、回调、Future/Promise、反应式扩展、协程
...
C# 将 `async` 以及 `await` 作为语法的一部分。而在 Kotlin 中，他们都只是库函数。之前的线程需要依靠操作系统的调度才能实现不同线程之间的切换。而使用协程的调度是由虚拟机负责的（用户级线程）
...
- `coroutineScope`与`runBlocking`类似，都是等待其内部所有相同作用域的协程结束后才会结束自己。两者的主要区别在于 `runBlocking` 方法会阻塞当前线程，而 `coroutineScope`不会。而且要注意的是`coroutineScope`是一个`suspend`函数。
  ...
  fun main() = runBlocking {
      ...
      // 由于coroutineScope是一个挂起函数，所以这里的log在coroutineScope之后输出。
