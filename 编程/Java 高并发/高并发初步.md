...
![image-20240211164822297](assets/image-20240211164822297.png)
...
一个分布式IM系统的设计，是对技术能力是一种非常极致的考验和训练。
...
`getChannel()`：`FileInputStream`、`FileOutputStream`、`RandomAccessFile`以及`Socket`等，它们都提供了`getChannel() `来方法获取通道。
...
2. 将通道注册到选择器中。
   ...
   这里的IO事件指的是Channel具备执行某个IO操作的条件，例如：
