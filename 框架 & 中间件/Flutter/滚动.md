...
3. Sliver 中对子组件（RenderBox）按需进行构建和布局，然后确认自身的位置、绘制等信息，保存在 SliverGeometry  中。
...
`PageView`会尽量扩展到父Widget所允许的最大尺寸，而且它的每个子组件的尺寸都会被紧约束为父级允许的最大尺寸。
...
- `controller`：接受一个`PageController`（页面控制器，继承于ScrollController）。它的方法有：

  - `jumpTo(300)`可以跳转300个逻辑像素（受pageSnapping属性影响），

  - `jumpToPage(3)`可以直接跳转到第4个页面。
  - `previousPage`（跳转到前一页）和`nextPage`（跳转到后一页）方法
...
| 未使用                              | 使用                                |
| ----------------------------------- | ----------------------------------- |
| ![图6-22](assets/6-22.8b8283a7.gif) | ![图6-23](assets/6-23.74a615bf.gif) |
