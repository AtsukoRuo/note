...
- `buildCounter`：设置计数器。
  ...
  InputDecoration中的counter属性也可以设置计数器。若两边都设置，counter属性则有优先权。
...
此外，`ExpansionTile`还可以接受一个`ExpansionTileController`控制器，它有两个方法`collapse()`、`expand()`
...
![The highlight is a rectangle the size of the box.](assets/ink_well.png)

- [onHover](https://api.flutter.dev/flutter/material/InkResponse/onHover.html)
- [onTap](https://api.flutter.dev/flutter/material/InkResponse/onTap.html)
