...
推荐使用RedisTemplate来操控Redis，它提供了一些模板方法
...
Jedis本身提供了一个连接池，即`JedisPool`对象。它还要搭配Apache的通用对象池工具`common-pool`一起使用
...
`GenericObjectPoolConfig`提供了很多参数，例如最大连接数、最大空闲连接数、连接活性检测等等
