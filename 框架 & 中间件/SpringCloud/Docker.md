...
![image-20240111145948065](assets/image-20240111145948065.png)
 ...
 In a VM, we must set how many physical resources we’ll need in advance;

With containers, we can also set the memory and the CPU that we’ll need by using Kubernetes, but this isn’t required. In case you don’t specify these values, the container engine will assign the necessary resources for the container to function correctly. 



**Docker Engine** is the core piece of the entire Docker system.  The engine is an application that follows the client-server pattern architecture. 
...
- **Docker client**：Docker users interact with Docker via a client
- **Docker Registr**：The location where Docker images are stored, These registries can be either public or private. The Docker Hub is the default place for the public registries, but you can also create your own private registry.
- **Docker images**：These are read-only templates with several instructions to create a Docker container. The images can be pulled from the Docker Registr, and you can use them, or you can modify them by adding additional instructions. Also, you can create new images by using a Dockerfile
...
~~~shell
$ docker run ostock/licensing-service:0.0.1-SNAPSHO
~~~
...
Docker Compose uses a YAML file for defining the services that are going to be launched. 
