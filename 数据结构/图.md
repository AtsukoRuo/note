...
2. $[dTime(u), fTime(u)] \cap [dTime(v), fTime(v)] = \empty$当且仅当v与u无祖先关系。（逆否命题）
...
1. 无向图的DFS树中并不存在跨边、前向边。所以，C的真子树只可能通过后向边与C的真祖先连通。注意无向图的树边都对应一条后向边。
2. 现在考察一个顶点$C$，若顶点C的删除导致某一颗榛子树与其真祖先之间无法连通，那么C必为关节点。显然，DFS树中的叶节点绝不可能是原图中的关节点，因为删除此类顶点并不影响DFS树的连通性。
